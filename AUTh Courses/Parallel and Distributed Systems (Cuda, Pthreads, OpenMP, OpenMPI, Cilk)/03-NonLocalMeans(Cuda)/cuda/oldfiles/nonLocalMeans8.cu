...
// 2d shared -> 1d?
// multi me to diko mou blockimg se geitonies stous registers?
...
__device__ __forceinline__ void getWeight(float blockImg[BSZ_HL][BSZ_HL], float foreignBlockImg[BSZ_HL][BSZ_HL], 
	float sigma, float weightSum[multi][multi], float fSum[multi][multi], float maxWeight[multi][multi]) {
				...
				for(int l = 0; l < BSZ; l++ ) {
					...
					for(int m = -NSZ2; m <= NSZ2; m++) { // Neighbourhoud
						for(int n = -NSZ2; n <= NSZ2; n++) {
							float diff = ( blockImg[(txM + NSZ2) + m][(tyM + NSZ2) + n] 
								  - foreignBlockImg[(k   + NSZ2) + m][(l   + NSZ2) + n] );
							// diff *= gaussDistW[ SER((n + MAX_NSZ / 2), (m + MAX_NSZ / 2), MAX_NSZ)];
							partialW += diff * diff * gaussDistW[ SER((n + MAX_NSZ / 2), (m + MAX_NSZ / 2), MAX_NSZ)];
						}
					}
					partialW = expf((-partialW / sigma));
					// if (!mx && !my && !k && !l) printf("%f\n",partialW);
					if ( partialW != 1 && maxWeight[mx][my] < partialW) {
						maxWeight[mx][my] = partialW;
					}
...
__device__ __forceinline__ void downloadAndCalculate(float blockImg[BSZ_HL][BSZ_HL], float foreignBlockImg[BSZ_HL][BSZ_HL],
	const float *inputImg, float sigma, float weightSum[multi][multi], float fSum[multi][multi], float maxWeight[multi][multi], int N, int I0) {
		getSharedBlock(foreignBlockImg, inputImg, I0, N);
		__syncthreads();
		getWeight(blockImg, foreignBlockImg, sigma, weightSum, fSum, maxWeight);
}
...
__global__ void nlm(float const *inputImg, float *outputImg, int N, float sigma) {
	...
	float weightSum[multi][multi], fSum[multi][multi], maxWeight[multi][multi]; // Weightsums for multiple pixels per thread.
	...
	downloadAndCalculate(blockImg, blockImg, inputImg, sigma, weightSum, fSum, maxWeight, N2, I0);
	for (char i = 0; i < GSZ; i++) { // gia kathe BLOCK stin arxiki eikona (X)
		for (char j = 0; j < GSZ; j++) {
			if ( !(by == j && bx == i) ) {
				...
				downloadAndCalculate(blockImg, foreignBlockImg, inputImg, sigma, weightSum, fSum, maxWeight, N2, I1);
			}
		}
	}
	// if(!tx && !ty && !bx && !by) 
	...
	for (int mx = 0; mx < multi; mx++) { 
		for (int my = 0; my < multi; my++) {
			...
			weightSum[mx][my] -= (1 - maxWeight[mx][my]);
			fSum[mx][my]      -= (1 - maxWeight[mx][my]) * blockImg[(txM + NSZ2)][(tyM + NSZ2)]; //swsto i tou blockImg?
